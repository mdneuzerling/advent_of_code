...
mod tests {
    ...
    fn individual_play_1() {
        let result = score_play(vec!['A', 'Y']);
        assert_eq!(result, 4);
    }

    #[test]
    fn individual_play_2() {
        let result = score_play(vec!['B', 'X']);
        assert_eq!(result, 1);
    }

    #[test]
    fn individual_play_3() {
        let result = score_play(vec!['C', 'Z']);
        assert_eq!(result, 7);
    }
...
static TO_DRAW: Lazy<HashMap<char, char>> =
    Lazy::new(|| HashMap::from([('A', 'X'), ('B', 'Y'), ('C', 'Z')]));

static TO_WIN: Lazy<HashMap<char, char>> =
    Lazy::new(|| HashMap::from([('A', 'Y'), ('B', 'Z'), ('C', 'X')]));

static TO_LOSE: Lazy<HashMap<char, char>> =
    Lazy::new(|| HashMap::from([('A', 'Z'), ('B', 'X'), ('C', 'Y')]));
...
fn score_play(play: Vec<char>) -> u32 {
    ...
    let outcome: char = play[1];
    let response: char;

    // we convert the X/Y/Z outcome into an X/Y/Z response
    // this lets us use the code copied from part A.
    if outcome == 'X' {
        response = *TO_LOSE.get(&opponent).unwrap();
    } else if outcome == 'Y' {
        response = *TO_DRAW.get(&opponent).unwrap();
    } else {
        response = *TO_WIN.get(&opponent).unwrap();
    }
...
fn is_draw(opponent: char, response: char) -> bool {
    let draw_response = TO_DRAW.get(&opponent).unwrap();
    return &response == draw_response;
}

fn is_winning_response(opponent: char, response: char) -> bool {
    let winning_response = TO_WIN.get(&opponent).unwrap();
    return &response == winning_response;
}
